/**
 * Cloudbase Adapter for QuickApp
 */
...
// Request类为平台特有的网络请求，必须实现post/get/fetch/upload/download 五个public接口
export class QAppRequest implements SDKRequestInterface {
  ...
  public fetch(options: IRequestOptions) {
    return new Promise((resolve, reject) => {
      try {
        const fetch = require('@system.fetch')
        const {
        ...
        fetch.fetch({
          url: formatUrl('https:', url),
          data: data ? JSON.stringify(data) : body,
          method,
          header: headers,
          responseType: 'json',
          success: function (res) {
...
// auth模块网络请求
export const requestFunction = function (url, options) {
  return new Promise((resolve, reject) => {
    try {
      const fetch = require('@system.fetch')
      const {
...
function genAdapter(options) {
  const adapter: SDKAdapterInterface = {
    ...
    // 获取平台唯一应用标识的api，微信小程序返回的是appid，快应用没有配置appid的概念，所以返回packageName为平台唯一标识
    // 查了代码，发现没有用到这个函数
    getAppSign: function () {
      try {
        const app = require('@system.app')
